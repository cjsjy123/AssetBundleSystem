...
{
	public class EditorCoroutineGroup
	{
		...
		static EditorCoroutineGroup instance = null;
		...
		static void CreateInstanceIfNeeded()
		{
			if (instance == null)
			{
				instance = new EditorCoroutineGroup();
				instance.Initialize();
			}
		}
		...
		static bool Process(EditorCoroutine coroutine)
		{
			...
			{
			    EditorCoroutine newCoroutine = new EditorCoroutine((IEnumerator) current, coroutine.ownerHash, coroutine.ownerType);
			    instance.GoStartCoroutine(newCoroutine);
                coroutine.currentYield = new YieldNestedCoroutine { coroutine = newCoroutine }; // new YieldWaitForIEnumator{CurrentEnumerator =(IEnumerator)current ,TaskCoroutine = coroutine};
            }
			else
			{
			    Debug.LogError("<" + coroutine.MethodName + "> yielded an unknown or unsupported type! (" + current.GetType() + ")");
			    coroutine.currentYield = new YieldDefault();
            }
			return true;
		}
